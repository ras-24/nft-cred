  ...
  const refreshNFTs = useCallback(async () => {
    try {
      ...
      if (walletAddress) {
        ...
        // Clear owned NFTs if wallet is disconnected
        setOwnedNFTs([]);
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900">
      ...
      <main className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="py-8">
          ...
          {/* Gallery welcome section - shown to all users */}
          <div className="mb-8 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 p-6 rounded-xl border border-blue-100 dark:border-indigo-800/30">
            <h2 className="text-xl font-semibold text-gray-800 dark:text-white mb-2">
              {walletAddress && showOwnedOnly ? 'Your NFT Collection' : 'Supported NFT Collections'}
            </h2>
            <p className="text-gray-700 dark:text-gray-300">
              {walletAddress && showOwnedOnly 
                ? 'Select one of your NFTs to use as collateral and get an instant loan.'
                : 'Browse all supported NFT collections. Connect your wallet to view and borrow against your owned NFTs.'}
            </p>
          </div>

          {/* Always show NFT grid, content depends on connection state */}
            ...
            {filteredNFTs.map((nft) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Overlay with buttons that appears on hover - only for connected users viewing owned NFTs */}
                  {walletAddress && showOwnedOnly && (
          ...
          {/* Show empty state only if there are no NFTs to display */}
          {filteredNFTs.length === 0 && (
            <div className="text-center py-8 text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 rounded-lg p-8">
              {walletAddress && showOwnedOnly 
                ? "You don't own any supported NFTs yet"
                : "No supported NFT collections available"}
            </div>
          )}
        </div>
      </main>
